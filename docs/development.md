...
スクリプトはコマンドの出力を解析する箇所がある。**実行するマシンの言語設定で
その表記が変わる**ため、入口で `LC_ALL=C.UTF-8` を固定している。

固定しないと何が起きるか。実機の日本語環境で `59-git-clis.sh` が
「gh の候補が見つかりません」で止まった。原因は `apt-cache policy` の出力:
...
`/Candidate:/` で拾っていたので日本語環境では空になっていた。
検証 VM が英語ロケールだったため、VM では最後まで気づけなかった。

`C` ではなく `C.UTF-8` にするのは、日本語を含む文字列の処理
(ログ出力や Python での読み書き) を壊さないため。

固定しているのは次の 4 か所で、`tests/static-checks.sh` が外れていないか見張る。

| ファイル | 対象 |
| --- | --- |
| `lib/common.sh` | 全モジュールと `install.sh` |
| `harness/asserts/lib.sh` | 全アサーション |
| `tests/state-manifest.sh` | 冪等性の比較 (sort の並び順も安定する) |
| `bin/first-run-wizard` | 各ツールの出力の解析 |

### `大きい出力 | grep -q` を書かない

モジュールは `set -euo pipefail` で動く。この状態で

```bash
if fc-list | grep -q "..."; then     # 危ない
```

と書くと、`grep -q` が一致した時点で終了して `fc-list` が SIGPIPE で死に、
**pipefail のせいでパイプライン全体が失敗扱いになる**。
一致しているのに条件が偽になるので、フォントの確認が毎回
「確認できませんでした」と警告する不具合になっていた。

`lib/common.sh` の `output_contains` を使う (パイプを使わず変数に取ってから判定する)。

```bash
if output_contains "Moralerspace Neon HW" fc-list; then
```

アサーション側で `bash -c "... | grep -q ..."` と書いている箇所は、
`bash -c` が pipefail なしの新しいシェルを起動するので影響を受けない。
...
### dconf のシステム既定値は「個人の設定が無いとき」しか効かない

`enabled-extensions` は 1 つのキーに配列を持つ形式なので、**ユーザー個人の値が
あるとシステム既定は丸ごと無視される**。Ubuntu を普通に使っていると
(拡張機能アプリを一度開くなど) 個人の値が入っていることがあり、その環境では
こちらで有効化したはずの拡張がひとつも読み込まれない。

実機でこれを踏んだ。症状は 2 つ同時に出た。

- **アプリがワークスペースに割り当てられない** (Auto Move Windows が動かない)
- **Ghostty で Alt+C を押すとターミナルが終了する**
  (xremap の GNOME 拡張が無いとフォーカス中のアプリを判別できず、
  設定した「ターミナルを除外する」が無視されて Alt+C が Ctrl+C になる)

検証 VM ではユーザーが個人設定を持っていなかったため、システム既定が効いていて
最後まで気づけなかった。

対処は 2 段構え。

1. `70-existing-users.sh` が、既存ユーザーの**個人設定にも**拡張を足す
   (個人の値が無いときは何もしない。あるときは既存の並びを保ったまま追記する)
2. `harness/asserts/90-services.sh` が、システム既定の拡張がすべて
   **実効値** (`gsettings get`) に入っていることを検証する。
   個人設定に上書きされていたらここで落ちる

### xremap は拡張が無ければ起動しない (fail safe)

上の不具合で分かったのは、**xremap は拡張が無いと安全側に倒れない**ということ。
アプリを判別できないまま keymap を全アプリに適用するので、
ターミナルの中で Ctrl+C や Ctrl+V が化けて操作を壊す。

そこで `files/bin/xremap-session` が、起動前に拡張の D-Bus 応答を待つ。
最大 60 秒待って現れなければ**起動しない**。ユニットは `Restart=always` なので、
ログインし直して拡張が読み込まれれば自動的に動き出す
(再試行が続くよう `StartLimitIntervalSec=0` を入れてある)。

これで「Mac 風のキーが効かない」(軽微) で止まり、
「ターミナルが壊れる」(重大) にはならない。
