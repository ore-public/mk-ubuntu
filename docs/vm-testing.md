...
- Brave がワークスペース 1、Ghostty がワークスペース 6 に自動配置されているか
...
- [ ] ターミナルのフォントが Moralerspace Neon HW になっている
- [ ] 日本語が半角幅で揃って表示される (罫線や表がずれない)
...
- [ ] Brave を起動するとワークスペース 1 に配置される
- [ ] Ghostty を起動するとワークスペース 6 に配置される
- [ ] Zoom を起動するとワークスペース 8 に配置される (amd64 実機)
- [ ] Discord を起動するとワークスペース 9 に配置される (amd64 実機)
- [ ] アプリを起動すると画面もそのワークスペースへ一緒に移動する
- [ ] 割当表にないアプリを起動しても画面は動かない
- [ ] 初回ウィザードはログイン直後に見える
      (ターミナルなので 6 へ移るが、画面も追随する)
...
- [ ] `mise use -g ruby@3.4` で Ruby が入り、`ruby -v` が切り替わる
