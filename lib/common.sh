...
# コマンドの出力を解析する箇所があるので、ロケールを固定して英語表記に揃える。
# 日本語環境では apt-cache policy が "Candidate:" ではなく "候補:" を返すため、
# これを固定しないと実行するマシンの言語設定によって動いたり動かなかったりする
# (実機の日本語環境で実際に踏んだ)。
# C ではなく C.UTF-8 にするのは、日本語を含む文字列の処理を壊さないため。
...
# output_contains <探す文字列> <コマンド> [引数...]
#
# コマンドの出力に文字列が含まれるかを返す。
# `大きい出力 | grep -q` は使わないこと: grep -q は一致した時点で終了するため
# 出力元が SIGPIPE で死に、set -o pipefail のせいでパイプライン全体が
# 失敗扱いになる (fc-list の確認が毎回 WARN になる不具合を実際に踏んだ)。
output_contains() {
  local needle="$1"
  shift
  local out
